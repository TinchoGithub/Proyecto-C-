...
namespace GestionTelefoniaCable
{
	...
	public class ControladorMenu
	{
		...
		public PeriodoConsumo hallarPeriodo (ArrayList periodos)
		{
			...
			{
				int actualInicio = DateTime.Compare(((PeriodoConsumo)periodos[i]).Periodo.FechaInicio, DateTime.Today);
				int vencimientoActual = DateTime.Compare(DateTime.Today, ((PeriodoConsumo)periodos[i]).Periodo.FechaFin);
				
				if (actualInicio <=0 && vencimientoActual <= 0)
				{     //MAYOR A 0
		...
		{
			string [] vector = {"Volver atras", "Imprimir ultima Factura", "Adelantar la Facturacion"};
		...
		private void facturarCliente(Cliente cliente){
			...
			if(cliente.Facturas.Count ==0 && fechaFin.Day == cliente.FechaAlta.Day){
				Console.ForegroundColor=ConsoleColor.Red;
				Console.WriteLine("El cliente aun no tiene facturas");
				Console.ForegroundColor=ConsoleColor.White;
				return;
			}
			if(fechaFin.Day == periodo.Periodo.FechaFin.Day){
				
				Console.ForegroundColor = ConsoleColor.Red;
				Console.WriteLine("El cliente ya fue factura el dia de hoy.");
				Console.ForegroundColor = ConsoleColor.White;
				Console.ReadKey();
				return;
			}
			...
			numeroFactura = ((Factura)cliente.Facturas[0] ).Numero + 1;
			periodo.Periodo.FechaFin = fechaFin;
			nuevaFactura = new Factura(numeroFactura, periodo);
		...
		private void ordenarFacturasPorFecha(ArrayList facturas)
		{
			...
			for(int i=0; i<(tam-1); i++)
			{
				...
				for(int j=i+1; j<tam; j++)
				{
					...
					var result = diferenciaFecha.TotalDays;
					if(result > 1)
						menor = j;
				}
				if(menor != i)
